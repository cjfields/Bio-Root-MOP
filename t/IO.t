...
is $temp_io->mode, 'rw', 'is a write handle';
ok($temp_io->close);
ok !$warn, 'no warnings';
...
ok(-e $TESTINFILE, 'file exists and is readable');
...

{
    ...
    use Bio::Root::MOP;
    with 'Bio::Role::IO::Handle';
    with 'Bio::Role::IO::Buffer_Unread';
    with 'Bio::Role::IO::Scalar';
}
...
##############################################
# tests IO handler/file base class (if you really want everything)
##############################################
...
SKIP: {
    ...
    isa_ok($rio, 'Bio::Root::IO::Base');
    
    does_ok($rio, 'Bio::Role::Root');
    does_ok($rio, 'Bio::Role::IO::Handle');
    does_ok($rio, 'Bio::Role::IO::File');
    does_ok($rio, 'Bio::Role::IO::Buffer');
